...
function Policeview(props) {
    const [userdata, setuserdata] = useState([]);
    const [time, setTime] = useState(new Date());
    const userapi = async () => {
        httpClient
          .GET("api-police-unit/getall", false, true)
          .then((resp) => {
            console.log(resp);
    ...
    const handleDelete=(e,rowData)=>{
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            ...
            httpClient
            .DELETE(`api-delete/unit/${id}`, false, true)
            .then((resp) => { 
  ...
  return (
    ...
    <div className="heading_line">
        <p className="text04">Police Unit</p>
      ...
      <Link to='../policeunit'>
      <button className="btn-details"  >Unit Details</button>
